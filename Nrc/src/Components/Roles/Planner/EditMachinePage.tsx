...
const EditMachinePage: React.FC<EditMachinePageProps> = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://nrc-backend-his4.onrender.com/api/jobs?nrcJobNo=${encodeURIComponent(searchTerm.trim())}`, {
  ...
  const handleJobSelect = async (job: Job) => {
    try {
      ...
      const response = await fetch(`https://nrc-backend-his4.onrender.com/api/job-planning/${encodeURIComponent(job.nrcJobNo)}`, {
        headers: { 'Authorization': `Bearer ${accessToken}` },
      });
  ...
  const handleSaveMachineAssignments = async (updatedSteps: JobPlanStep[]) => {
    try {
      ...
      const response = await fetch(`https://nrc-backend-his4.onrender.com/api/job-planning/${encodeURIComponent(selectedJob!.nrcJobNo)}`, {
        method: 'PUT',
        headers: {
