...
import { type Job, type JobStep, type Machine } from '../Types/job.ts'; // Adjust path as needed
import SelectDemandModal from '../modal/SelectDemandModal.tsx'; // Will create this
import AddStepsModal from '../modal/AddStepsModal.tsx'; // Will create this
import MachineAssignedModal from '../modal/MachineAssignedModal.tsx'; // Will create this
...
const MoreInformationForm: React.FC<MoreInformationFormProps> = ({ job, onSave, onClose }) => {
  ...
  const [selectedMachine, setSelectedMachine] = useState<Machine | null>(null); // Store the full machine object
  ...
  // Effect to set selectedMachine if job.machineId already exists
  useEffect(() => {
    if (job.machineId) {
      // In a real app, you'd fetch the machine details by ID here
      // For now, we'll just set a placeholder or assume the ID is enough
      setSelectedMachine({ id: job.machineId, machineType: '', description: '', unit: '', machineCode: '', type: '', capacity: 0, remarks: null, status: '', isActive: true, createdAt: '', updatedAt: '', jobs: [] });
    }
  }, [job.machineId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Payload for updating main Job object (jobDemand, machineId)
      const jobUpdatePayload: Partial<Job> = {
        jobDemand: jobDemand,
        machineId: selectedMachine.id, // Update machineId on the main job
      };

      // Payload for job-planning endpoint (nrcJobNo, jobDemand, steps)
      const jobPlanningPayload = {
        ...
        steps: selectedSteps.map(step => ({
          ...
          machineDetail: step.machineDetail || selectedMachine.description || selectedMachine.machineCode, // Use machine description or code if available
        })),
      };

      await onSave(jobUpdatePayload, jobPlanningPayload);
