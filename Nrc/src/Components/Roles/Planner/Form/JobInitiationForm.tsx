...
const JobInitiationForm: React.FC<JobInitiationFormProps> = ({ onJobUpdated }) => {
  ...
  const createJobPlanFromPayload = async (completedJob: Job, jobPlanningPayload: any, accessToken: string) => {
    try {
      ...
      const jobPlanResponse = await fetch('https://nrc-backend-his4.onrender.com/api/job-planning/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${accessToken}`,
      ...
      if (jobPlanResponse.ok) {
        ...
        const statusUpdateResponse = await fetch(`https://nrc-backend-his4.onrender.com/api/jobs/${completedJob.nrcJobNo}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${accessToken}`,
